...
export const getBrowserName = userAgent => {
  ...
  if (compare('MSIE') || ie11) {
    ...
    if (userAgent.indexOf('Edge') > -1) {
      ...
      const regStr_saf = /chrome\/([\d.]+)/
      const Chrome = userAgent.toLowerCase().match(regStr_saf)
      const version = (Chrome && Chrome[1]) || ''
      return 'Chrome/' + version
    }
  } else if (compare('Opera') && !ie11) {
    return 'Opera'
  } else if (compare('Safari') && !ie11) {
    const regStr_saf = /version\/([\d.]+).*safari/
    const Safari = userAgent.toLowerCase().match(regStr_saf)
    const version = (Safari && Safari[1]) || ''
    // Safari浏览器版本小于10返回错误ie页面
